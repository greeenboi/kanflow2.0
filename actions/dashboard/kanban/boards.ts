...
export async function getUserBoards(userId: number): Promise<Board[]> {
    const db = await Database.load('sqlite:kanflow.db');
    const sql = 'SELECT * FROM boards WHERE user_id = ? AND is_archived = FALSE ORDER BY last_updated DESC';
...
export async function archiveBoard(id: number): Promise<void> {
    const db = await Database.load('sqlite:kanflow.db');
    const sql = 'UPDATE boards SET is_archived = TRUE, last_updated = CURRENT_TIMESTAMP WHERE id = ?';
