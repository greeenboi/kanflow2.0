        ...
        <DropdownMenuSub>
          ...
          <DropdownMenuSubContent>
            {colorSchemes.map((scheme) => (
              <DropdownMenuItem 
                key={scheme.value}
                onClick={() => setColorScheme(theme?.includes("dark") ? "dark" : "light", scheme.value)}
                className="gap-2"
              >
                <div className={cn(isCurrentScheme(scheme.value) ? "border-2 border-white rounded-md " : "" ,`h-4 w-4 rounded ${scheme.color}`)} />
                <span>{scheme.name}</span>
                {isCurrentScheme(scheme.value) && <Check className="h-4 w-4 ml-auto" />}
              </DropdownMenuItem>
            ))}
          </DropdownMenuSubContent>
        </DropdownMenuSub>
